...
<body>
	...
	<%	List<Furto> listaFurti = Furto.getFurtiAttiviUtente(user); 
		if (!listaFurti.isEmpty()){ %>
			<table order="1" cellpadding="1" cellspacing="5">
				...
				<tbody>
					<%	for (Furto f : listaFurti) { %>
						<tr>
							<td><input type="radio" name="furtoLiveSel" value="<%=f.getId()%>" required></td>
							<td><%=f.getTarga()%></td>
							<td><%=f.getData()%></td>
							<td><%=f.getOra()%></td>
						</tr>
			...
			<hr/>
			<input type="submit" name="azione" value="Live Tracking">
		<% } else { %>
			<h1>Nessun furto in corso!</h1>
			<h2>I tuoi veicoli son al sicuro.</h2>
			<hr/>
		<% } %>
	</form>
	<br/>
